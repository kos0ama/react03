...
//const clientId = 'babbc081-0761-4f16-8f56-071aa402ebcb'; //Tutorial　ID
const clientId = 'd03eb90b-4753-4144-be08-da2890cfa679'; //ama_implicit トークンの暗黙的な付与（ブラウザー
//const clientId = 'd625ef0c-0931-4714-b7b8-7211c6218b8f'; //ama_test クライアント資格情報
//const clientId = '4c9272c7-46f4-440d-ae7b-e87a5b0dba19'; //ama_cti コード認証 won’t work if you use a client ID that’s configured for the auth code grant.
...
$(document).ready(() => {
	...
	client
		...
		.then((topicSubscriptions) => {
			...
			CONVERSATION_LIST_TEMPLATE = Handlebars.compile(
				$('#entry-template').html()
			);
...
function handleNotification(message) {
	...
	if (notification.topicName.toLowerCase() === 'channel.metadata') {
		...
		notification.topicName.toLowerCase() !== conversationsTopic.toLowerCase()
	) {
		// Unexpected topic
	...
	else conversationList[notification.eventBody.id] = notification.eventBody;

	// Update UI
	$('#call-table').html(
		CONVERSATION_LIST_TEMPLATE(Object.values(conversationList))
	);
}
